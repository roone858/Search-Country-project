...
let starting = document.getElementById("starting-word");
let az = document.getElementById("az");
let pop = document.getElementById("pop");
let findChar = document.getElementById("find-char");
...
starting.addEventListener("click", function () {
  ...
  starting.style.backgroundColor = " rgb(255, 171, 36)";
  findChar.style.backgroundColor = " rgba(240, 248, 255, 0.239)";
});
findChar.addEventListener("click", function () {
  clicked = 2;
  container.innerHTML = "";
  print(countries);
  findChar.style.backgroundColor = " rgb(255, 171, 36)";
  starting.style.backgroundColor = " rgba(240, 248, 255, 0.239)";
});
...
function print() {
  ...
  if (clicked == 0) {
    label.textContent = "you must chose an option";
  } else {
    if (clicked == 1) {
      label.textContent = "";
      regexp = new RegExp(`^${search.value}`, "i");
    } else if (clicked == 2) {
      label.textContent = "";
      regexp = new RegExp(`${search.value}`, "i");
    }
    for (const country of countries) {
      if (container.innerHTML.includes(country.name)) {
        continue;
      } else if (search.value != "" && regexp.test(country.name)) {
        container.innerHTML += `
       <div>
  <img style="width:70%;height="40%" src=${country.flag}>
        <h3>${country.name}</h3> 
...
az.onclick = function () {
  if (countries[0].name.startsWith("A")) {
